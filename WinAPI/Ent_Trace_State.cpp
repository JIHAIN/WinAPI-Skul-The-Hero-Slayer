...
Ent_Trace_State::Ent_Trace_State()
	:AccTime(0.f)
{
}
...
void Ent_Trace_State::FinalTick()
{
		...
		if (vDir.x > 0)
		{
			...
			MonAni->Play(L"Ent_Move_Right", true);
		}
		else
		{
			CAnimator* MonAni = pMonster->GetMonsterAnimator();
			MonAni->Play(L"Ent_Move_Left", true);
		}
