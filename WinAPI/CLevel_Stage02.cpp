...
void CLevel_Stage02::Init()
{
	...
	// 플랫폼 설치
	wstring platformDataPath = CPathMgr::Get()->GetContentPath() + L"platform\\Stage2.map";
	...
	pTileMap->UseLoad(L"Stage2_Tile");
	AddObject(pTile, LAYER_TYPE::TILE);

	// 오브젝트 위치 설정
	CPlayer* pPlayer = new CPlayer;
	pPlayer->SetName(L"Player");
	pPlayer->SetPos(191.f, 1500.f);
	...
	pSword->SetPos(1200.f, 1500.f);
	pSword->SetInitPos(pSword->GetPos());
	AddObject(pSword, LAYER_TYPE::MONSTER);

	pSword = new Sword_Monster;
	pSword->SetName(L"Monster");
	pSword->SetPos(1300.f, 1500.f);
	pSword->SetInitPos(pSword->GetPos());
	AddObject(pSword, LAYER_TYPE::MONSTER);

	pSword = new Sword_Monster;
	pSword->SetName(L"Monster");
	pSword->SetPos(1400.f, 1500.f);
	pSword->SetInitPos(pSword->GetPos());
	AddObject(pSword, LAYER_TYPE::MONSTER);

	pSword = new Sword_Monster;
	pSword->SetName(L"Monster");
	pSword->SetPos(1500.f, 1500.f);
	pSword->SetInitPos(pSword->GetPos());
	AddObject(pSword, LAYER_TYPE::MONSTER);

	pSword = new Sword_Monster;
	pSword->SetName(L"Monster");
	pSword->SetPos(2200.f, 330.f);
	pSword->SetInitPos(pSword->GetPos());
	AddObject(pSword, LAYER_TYPE::MONSTER);

	pSword = new Sword_Monster;
	pSword->SetName(L"Monster");
	pSword->SetPos(2400.f, 330.f);
	pSword->SetInitPos(pSword->GetPos());
	AddObject(pSword, LAYER_TYPE::MONSTER);
	...
	pSniper->SetPos(2800, 330);
	pSniper->SetInitPos(pSniper->GetPos());
	AddObject(pSniper, LAYER_TYPE::MONSTER);

	pSniper = new Sniper_Monster;
	pSniper->SetName(L"Monster");
	pSniper->SetPos(2600, 330);
	pSniper->SetInitPos(pSniper->GetPos());
	AddObject(pSniper, LAYER_TYPE::MONSTER);


	Mage_Monster* pMage = new Mage_Monster;
	pMage->SetName(L"Monster");
	pMage->SetPos(1570, 1250);
	pMage->SetInitPos(pMage->GetPos());
	AddObject(pMage, LAYER_TYPE::MONSTER);

	pMage = new Mage_Monster;
	pMage->SetName(L"Monster");
	pMage->SetPos(1900, 1050);
	pMage->SetInitPos(pMage->GetPos());
	AddObject(pMage, LAYER_TYPE::MONSTER);

	pMage = new Mage_Monster;
	pMage->SetName(L"Monster");
	pMage->SetPos(2000, 1050);
	pMage->SetInitPos(pMage->GetPos());
	AddObject(pMage, LAYER_TYPE::MONSTER);

	BigEnt_Monster* pBigEnt = new BigEnt_Monster;
	pBigEnt->SetName(L"Monster");
	pBigEnt->SetPos(1550, 850);
	pBigEnt->SetInitPos(pBigEnt->GetPos());
	AddObject(pBigEnt, LAYER_TYPE::MONSTER);


	Door_1* Door = new Door_1;
	Door->SetPos(3380.f, 330.f);
